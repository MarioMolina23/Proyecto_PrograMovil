...
class Bus_Periferica : AppCompatActivity(), OnMapReadyCallback, GoogleMap.OnMyLocationButtonClickListener, GoogleMap.OnMyLocationClickListener {
    ...
    private fun createMarker() {
        val favoritePlace = LatLng(9.925209205959037, -84.09158534328793)
        val m = map.addMarker(MarkerOptions().position(favoritePlace).title("Parada de buses Periférica"))
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            ...
            R.id.menu_map -> {
                startActivity(Intent(this, Bus_Periferica::class.java))
                true
            }
            R.id.menu_contacto -> {
