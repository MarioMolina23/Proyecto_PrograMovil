...
class Bus_Escazu : AppCompatActivity(), OnMapReadyCallback, GoogleMap.OnMyLocationButtonClickListener, GoogleMap.OnMyLocationClickListener {
    ...
    private fun createMarker() {
        val favoritePlace = LatLng(9.93511184434152, -84.08561331283508)
        val m = map.addMarker(MarkerOptions().position(favoritePlace).title("Terminal de Buses Escazú"))
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            ...
            R.id.menu_map -> {
                startActivity(Intent(this, Bus_Escazu::class.java))
                true
            }
            R.id.menu_contacto -> {
